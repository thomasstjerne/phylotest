...
const parseNexus = (nexusString) => {
    ...
    Scanner.prototype.ParseComment = function () {
      ...
      this.buffer += this.str.charAt(this.pos);
      this.comments.push(this.buffer);
      // console.log('[' + this.buffer + ']');
    };
    ...
    Scanner.prototype.ParseNumber = function () {
      ...
      while (
        ...
        if (isNumber(this.str.charAt(this.pos))) {
          ...
          (this.str.charAt(this.pos) == 'E' || this.str.charAt(this.pos) == 'e') &&
          (state == NumberTokens.digit || state == NumberTokens.fraction)
        ) {
    ...
    //----------------------------------------------------------------------------------------------
    Scanner.prototype.ParseMatrix = function () {
      this.buffer = '';
      //	console.log(Array(500).fill().map((element, index) => index).map(e => this.str.charAt(this.pos + e)).join(''))
      while (this.str.charAt(this.pos) != ';' && this.pos < this.str.length) {
        this.buffer += this.str.charAt(this.pos);
        this.pos++;
      }
    ...
    //----------------------------------------------------------------------------------------------
    Scanner.prototype.ParseTaxaLabels = function () {
      this.buffer = '';
    ...
    function NexusReader() {
      ...
      this.nexusBlocks = ['taxa', 'characters', 'trees', 'codons'];
    }
    ...
    NexusReader.prototype.GetCommand = function () {
      ...
      if (t == TokenTypes.String) {
        //	console.log(this.buffer.toLowerCase())
    ...
    function parse_nexus(str) {
      ...
      if (blockname == 'taxa') {
        ...
        //console.log(`Taxa Command  ${command}`)
  
        nexus.taxablock = {};
        while (command != 'end' && command != 'endblock' && nx.error == NexusError.ok) {
          switch (command) {
            case 'taxlabels':
              //console.log(`Taxa Command  ${command}`)
              // console.log(nx.buffer)
              nx.ParseTaxaLabels();
              nexus.taxablock.treelabels = nx.buffer
                .split('\n')
                .map(function (l) {
                  return l.trim();
                })
              ...
              //	console.log(nexus.taxablock.treelabels)
              nx.SkipCommand();
      ...
      if (blockname == 'characters') {
        command = nx.GetCommand();
        // console.log(`Character Command  ${command}`)
  
        nexus.charactersblock = {};
        while (command != 'end' && command != 'endblock' && nx.error == NexusError.ok) {
          switch (command) {
            case 'matrix':
              ...
              nx.ParseMatrix();
              //console.log(nx.buffer)
              nexus.charactersblock.matrix = nx.buffer
                .trim()
                .split('\n')
                .map(function (l) {
                  var splitted = l.split(/\s+/);
                  return {
                    taxon: splitted.slice(0, -1).join(' '),
                    sequence: splitted[splitted.length - 1],
                  };
                })
                .filter(function (l) {
                  return !!l;
                });
              //	console.log(nexus.charactersblock.matrix)
              nx.SkipCommand();
              command = nx.GetCommand();
              break;
          ...
          switch (command) {
            case 'translate':
              ...
              while (!done && nx.error == NexusError.ok) {
                ...
                if (
                  [TokenTypes.Number, TokenTypes.String, TokenTypes.QuotedString].indexOf(t) != -1
                ) {
                  var otu = nx.buffer;
                  t = nx.GetToken();
      ...
      // Comments starting with ! are meant to be printed
      nexus.comments = nx.comments
        .filter(function (c) {
          return c.charAt(1) === '!';
        })
        .map(function (c) {
          return c.slice(2, -1).trim();
        });
  
      return nexus; // {...nexus, comments: nx.comments.filter(c => c.charAt(1) === '!').map(c => c.slice(2, -1).trim())};
    }
    return parse_nexus(nexusString);
  };
